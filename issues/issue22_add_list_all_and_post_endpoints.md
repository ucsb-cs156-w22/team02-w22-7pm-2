Create `UCSBSubjectController`, add `GET` (index) and `POST` (create)

# Acceptance Criteria:

- [ ] There is a controller file `UCSBSubjectController.java`
      in the expected directory.
- [ ] In `UCSBSubjectController.java` there is 
      code for a `GET /api/UCSBSubjects/all` endpoint 
      that returns a JSON list of all subreddits in the database.
      (We sometimes call this an *index* action since it lists all
      items in the database.)
- [ ] In `UCSBSubjectController.java` there is 
      code for a `POST /api/UCSBSubjects/post` endpoint
